...
(() => {
  ...
  /* No token wired means no token section at all. An empty panel promising a
     chart is worse than saying nothing, and the nav link would go nowhere.
     Wiring an address with script/wire-token.mjs turns the whole thing on. */
  if (!CONFIG.mainToken.address) {
    $("token").remove();
    document.querySelector('header nav a[href="#token"]')?.remove();
  } else {
